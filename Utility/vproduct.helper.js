...
function BuyProduct() {
    ...
    location.href = "/Vshop/SubmmitOrder.aspx?buyAmount=" + $("#buyNum").val() + "&productSku=" + $("#hiddenSkuId").val() + "&from=signBuy&ReferralId=" + getParam("ReferralId");
}
...
function BuyProductToCart() {
    $.ajax({
        url: "/API/VshopProcess.ashx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "AddToCartBySkus", quantity: parseInt($("#buyNum").val()), productSkuId: $("#hiddenSkuId").val(),Templateid:$("#vProductDetails_litTemplate").val(),categoryid: $("#vProductDetails_litCategoryId").val() },
        async: false,
        success: function (resultData) {
            ...
            if (resultData.Status == "OK") {
                ...
                if (resultData.Status == "2") {
                    ...
                    if (!isWeiXin())
                        location.href = "/UserLogin.aspx?returnUrl=" + url + "";
                    else
