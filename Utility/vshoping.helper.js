...
function deleteCartProduct(obj) {
    ...
    $.ajax({
        ...
        data: { action: "DeleteCartProduct", skuId: $(obj).attr("skuId"), type: type },
...
function submmitorder() {
    ...
    $.ajax({
        ...
        data: {
            ...
            productSku: getParam("productSku"), buyAmount: getParam("buyAmount"), from: getParam("from"), shiptoDate: $("#selectShipToDate").val(), groupbuyId: $('#groupbuyHiddenBox').val(), remark: remark
        },
        success: function (resultData) {
            maskayer(1);
            if (resultData.Status == "OK") {
