                        ...
                        <UI:Grid runat="server" ID="grdProducts" Width="100%" AllowSorting="true" ShowOrderIcons="true"
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="操作" ItemStyle-Width="10%" HeaderStyle-Width="95" HeaderStyle-CssClass=" td_left td_right_fff">
                                    <ItemTemplate>
                                        <button type="button" class="btn btn-default" value='<%= Globals.DomainName+"/ProductDetails.aspx?productId=" %><%#Eval("ProductId") %>' onclick="gotovalue(this.value,'<%#Server.HtmlEncode(Eval("ProductName").ToString()) %>','<%#Server.HtmlEncode(Eval("ShortDescription").ToString()) %>','<%#Server.HtmlEncode(Eval("ThumbnailUrl40").ToString()) %>');">选取</button>
                        ...
                        <UI:Grid ID="grdTopCategries" DataKeyNames="CategoryId" runat="server" ShowHeader="true"
                            ...
                            <Columns>
                                <asp:TemplateField HeaderText="分类名称" HeaderStyle-Width="80%" HeaderStyle-CssClass="td_right td_left">
                                    <ItemTemplate>
                                        <a href='<%# "ProductOnSales.aspx?CategoryId="+Eval("CategoryId")%>'>
                                            <%#Eval("Name") %></a>
                                    </ItemTemplate>
                                </asp:TemplateField>
