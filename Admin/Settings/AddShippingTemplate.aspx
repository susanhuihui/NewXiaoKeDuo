            ...
            if (postData.FreeShip == 0) {
                ...
                $("input[name='ShippingType']:checked").each(function () {
                    ...
                    if (! /(^\d+$)/.test(SpecifyItem.FristNumber) || ! /(^\d+$)/.test(SpecifyItem.AddNumber)) {
                        ...
                        if (err == 0) {
                            HiTipsShow("[" + keyValue + "]：默认运费单位请填写数值！", 'error');
                            err = 1;
                        }
                        return;
                    };
                    if (! /^\d+(\.\d{2})?$/.test(SpecifyItem.FristPrice) || ! /(^\d+$)/.test(SpecifyItem.AddPrice)) {
                    ...
                    if ($Region.length > 0) {
                        ...
                        if ($RegionItems.length > 0) {
                            ...
                            $RegionItems.each(function () {
                                ...
                                if (! /^\d+(\.\d{2})?$/.test(SpecifyItem_Regions.FristPrice) || ! /(^\d+$)/.test(SpecifyItem_Regions.AddPrice)) {
