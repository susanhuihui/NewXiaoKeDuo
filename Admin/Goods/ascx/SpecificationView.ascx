                     ...
                     <asp:TemplateField HeaderText="操作" HeaderStyle-CssClass="td_right td_left" ItemStyle-Width="20%">
                         ...
                         <ItemTemplate>
	                         <span class="submit_tiajia"><a href="javascript:void(0)" onclick="ShowAddSKUValueDiv('<%# Eval("AttributeId") %>','<%# Eval("UseAttributeImage") %>', '<%# Eval("AttributeName") %>');">添加规格值</a></span>	                         	                        
 	                         <span class="submit_bianji"><asp:HyperLink  ID="lkbViewAttribute" runat="server" Text="编辑" NavigateUrl='<%# Globals.GetAdminAbsolutePath(string.Format("Goods/EditSpecificationValues.aspx?TypeId={0}&AttributeId={1}&UseAttributeImage={2}",Eval("TypeId"),Eval("AttributeId"),Eval("UseAttributeImage")))%>' ></asp:HyperLink></span> 
