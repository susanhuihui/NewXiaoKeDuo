    ...
    <UI:Grid CssClass="table table-hover mar table-bordered" ID="grdAttribute" runat="server" ShowHeader="true" AutoGenerateColumns="false" DataKeyNames="AttributeId" HeaderStyle-CssClass="table_title" GridLines="None"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作" HeaderStyle-CssClass="td_right td_left" ItemStyle-Width="25%">
                ...
                <ItemTemplate>
                    <span class="submit_tiajia"><a href="javascript:void(0)" onclick="ShowAddSKUValueDiv('<%# Eval("AttributeId") %>','<%# Eval("AttributeName") %>');">添加属性值</a></span> <span class="submit_bianji">
                        <asp:HyperLink ID="lkbViewAttribute" runat="server" Text="编辑" NavigateUrl='<%#Globals.GetAdminAbsolutePath(string.Format("Goods/EditAttributeValues.aspx?TypeId={0}&AttributeId={1}",Eval("TypeId"),Eval("AttributeId")))%>'></asp:HyperLink></span>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-horizontal">
                    ...
                    <small class="help-block" style="margin-left:130px;">扩展属性的值，多个属性值可用“,”号隔开，每个值的字符数最多15个字符</small>
                </div>
    ...
    function ShowAddSKUValueDiv(attributeId, attributename) {
        ...
        $('#aspnetForm').formvalidation({
            ...
            'ctl00$ContentPlaceHolder1$attributeView$txtValueStr': {
                validators: {
                    notEmpty: {
                        message: '扩展属性的值，多个属性值可用“,”号隔开，每个值的字符数最多15个字符'
                    } 
                }
            }
