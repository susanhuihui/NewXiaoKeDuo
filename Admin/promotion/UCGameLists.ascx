...
<script>
    ...
    function CheckAllDel() {
        ...
        return confirm('确定要执行该删除操作吗？删除后将不可以恢复！');
    }
</script>
<style type="text/css">
        ...
        <div id="search" >
            ...
            <div class="form-inline" style="margin-bottom: 10px; margin-top: 10px;"
                ...
                <asp:Button ID="btnDel" runat="server" Text="批量删除" CssClass="btn btn-danger resetSize"
                    Style="margin-left: 20px;" OnClick="btnDel_Click" OnClientClick="return CheckAllDel()" />
            </div>
        </div>
        <UI:Grid ID="grdGameLists" runat="server" ShowHeader="true" AutoGenerateColumns="false"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="有效期" ShowHeader="true" HeaderStyle-Width="200">
                    <ItemStyle VerticalAlign="Middle" HorizontalAlign="Center" />
                    <ItemTemplate>
                        <%# Eval("BeginTime","{0:yyyy-MM-dd HH:mm:ss}")%> 至 
                ...
                <asp:TemplateField HeaderText="操作" HeaderStyle-CssClass="border_top border_bottom">
                    <ItemStyle CssClass="spanD spanN" VerticalAlign="Middle" HorizontalAlign="Right"
                        Width="340" />
                    <ItemTemplate>
                        <span>
                        ...
                        <span class="submit_jiage"><a href="<%# GetEditUrl(Eval("GameId").ToString()) %>"
                            class="btn btn-primary resetSize">编辑</a></span>
                        <span title="FinishSpan" class="submit_jiage">
                            <asp:LinkButton runat="server" ID="FinishBtn"
                                CssClass="btn btn-warning resetSize" CommandName="Update" Text="结束" OnClientClick="return confirm('确定要结束游戏！');"></asp:LinkButton>
                        </span>
