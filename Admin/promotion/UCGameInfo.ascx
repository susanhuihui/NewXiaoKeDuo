...
<script type="text/javascript">
    ...
    function BeforeSave_Game() {
        ...
        if ($('.allradio input:checked').length == 0) {
            ShowMsg("请选择适用会员！")
            return;
        }
        ...
        <div class="form-horizontal clearfix">
            <div class="form-group setmargin">
                ...
                <div class="form-inline col-xs-9">
                    <asp:TextBox runat="server" class="form-control resetSize" ID="txtGameTitle"></asp:TextBox>
            ...
            <div class="form-group">
                ...
                <div class="form-inline journal-query col-xs-9">
                    <div class="form-group">
                        <asp:TextBox runat="server" TextMode="MultiLine" class="form-control inputtext" ID="txtDescription"></asp:TextBox>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="form-group setmargin">
                ...
                <div class="form-inline col-xs-9">
                    <div class="resetradio mb5 pt3">
                        <label>
                            <input type="checkbox" name="allApplyMembers" value="0" id="allMember" <% if (isAllCheck)
                                                                                                      { %>
                                checked="checked" <%}%> />全部会员</label>
                    </div>
                    <div class="resetradio mb5 pt3 allradio">
                        <% foreach (var item in MemberGrades)
                           { %>
                        <label class="mr20">
                            <input type="checkbox" name="applyMembers" value="<%=item.GradeId %>" <% if (item.IsCheck)
                                                                                                     { %>
                                checked="checked" <%}%> /><%=item.Name %></label>
                        <% }%>
                    </div>
                </div>
            </div>
            <div class="form-group setmargin">
                <label class="col-xs-3 pad resetSize control-label"><em>*</em>&nbsp;&nbsp;消耗积分：</label>
                <div class="form-inline col-xs-9">
                    <asp:TextBox runat="server" name="canTest" class="form-control resetSize" ID="txtNeedPoint"
                        Text="0"></asp:TextBox> 积分/次
                     <small>用户每次参与活动需要消耗积分，值为0时不消耗</small>
                </div>
            </div>
            <div class="form-group setmargin">
                <label class="col-xs-3 pad resetSize control-label"><em>*</em>&nbsp;&nbsp;参与送积分：</label>
                <div class="form-inline col-xs-9">
                    <asp:TextBox runat="server" name="canTest" class="form-control resetSize" ID="txtGivePoint"  Text="0"></asp:TextBox>积分/次
